...
initialization. To make it worse, 
...
called Hybrid Learning Net(HLN), 
with Self Organizing Maps(SOMs) 
aided to learn from samples in 
both unsupervised and supervised 
way, targeting to achieve a much 
faster net learning for general 
...
impressive promotion on both 
training and testing phases
of general applications, and 
we have unexpectedly discovered 
some interesting facts about 
neural network trainings, 
such as neuron activation 
sparsity is somehow strongly 
related to the training loss 
...
technique is way too complicated, 
and not neural-network-integreted
\cite{socher2011semi}.

Here we introduce a new architecture
...
decoupled hidden layer consisting
a Self Organizing Map(SOM) and a 
fully connected layer, training in
...
\item $f(x)$ is the embedding function, it tries to produce
a vector from $x_i$, similar to that of $x_j$ with $W_{ij}=0$,
and disimilar with $W_{ij}=1$.
\item $W_{ij}\in \mathbf{R}^d$ is the similarity label matrix 
