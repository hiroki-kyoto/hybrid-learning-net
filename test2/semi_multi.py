...
def main():
	...
	uepn = 10 # unsupervised epoch number
	...
	if net.type == 'HLNN':
		# if SOMs stablized, then FCNs too;
		# and if FCNs stablized, then SOMs too.
		

	if net.type=='HLNN':
		print('unsupservised training...')	
		
		for i in xrange(0, unum, 1):
			v = ims1[useq[i],:]
			uerr[i/train,:] += net.train(v,[])
